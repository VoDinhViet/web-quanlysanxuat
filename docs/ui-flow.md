...
9 nhóm sidebar (`AppSidebar.tsx`), 23 mục menu, 72 màn hình. "Lối vào" chỉ ghi cho màn con
...
| Quản lý kho      | Nhập kho `/manage/inventory-receipts`             | `create`, `create-receipt`, `$id`, `$id/update`          | nút "Tạo phiếu nhập"; **phiếu nhập TP không có route create riêng** — `[TỰ ĐỘNG]` sinh ở `PENDING_RECEIPT` khi OQC đóng coverage Job (xem F1)       |
...
flowchart LR
    ...
    L -.->|TỰ ĐỘNG, đóng coverage OQC| M["Phiếu nhập TP PENDING_RECEIPT /inventory-receipts/$id"]
    M -->|"Xác nhận nhập kho"| N["Phiếu nhập TP POSTED"]
...
| 11  | _(tự động)_           | OQC đóng coverage Job (`closeJobIfQcCovered`)       | —                                                       | phiếu nhập TP sinh sẵn `PENDING_RECEIPT` (1 dòng, SL kế hoạch)               |
| 12  | Chi tiết phiếu nhập   | "Xác nhận nhập kho"                                 | `/manage/inventory-receipts/$id`                        | phiếu nhập → `POSTED`, tồn kho TP tăng                                       |
...
flowchart TD
    ...
    OSIN --> JOB
    JOB --> OQC["OQC /oqc"]
    OQC -.->|TỰ ĐỘNG| RCPT_FG["Nhập kho TP /inventory-receipts (tự sinh PENDING_RECEIPT)"]
...
| --- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| F1  | ~~Chặn luồng~~ ✅ Đã xử lý 2026-09-03   | `/manage/inventory-receipts/create-from-job` mồ côi — không route/nút nào trong UI dẫn tới, đây là route duy nhất trong 71 route rơi vào tình trạng này. Người dùng thường (không gõ URL) không có cách nào tạo phiếu nhập kho thành phẩm → Job không bao giờ tự đóng `COMPLETED` qua UI.                                                                                                                                                                                                         | ✅ **Xác nhận** lúc test (31/08). Kiểm tra header Job (`JOB0002`) ở cả trạng thái `Đang SX` lẫn `Chờ giao hàng` (`WAITING_DELIVERY`) — không có nút "Nhập kho thành phẩm" ở bất kỳ trạng thái nào; route vẫn hoạt động đúng khi gõ URL tay kèm `?productionJobId=...`. **Đã hết hiệu lực từ 01/09** (BE commit `3c9a6e7`, be-quanlysanxuat): `closeJobIfQcCovered` tự sinh phiếu nhập TP `PENDING_RECEIPT` ngay khi OQC đóng coverage Job — không cần thao tác UI nào để tạo phiếu nữa. Route mồ côi đã xoá 03/09. |
| F2  | ~~Rác tài liệu~~ ✅ Đã xử lý 2026-09-03 | 2 comment mâu thuẫn về lối vào màn nhập TP: `InventoryReceiptCreateFromJobForm.tsx:24-27` (nói có nút trên header Job) vs `ProductionJobDetailHeader.tsx:33-35` (nói cố ý không có, vì nhập kho tạo từ module Inventory Receipts). Thực tế module Inventory Receipts cũng không link tới. Nên sửa 1 trong 2 comment cho khớp thực tế, hoặc bổ sung entry point thật (khuyến nghị: thêm nút trên header Job khi `WAITING_DELIVERY`, theo đúng ý định ban đầu của `ProductionJobDetailHeader.tsx`). | ✅ **Xác nhận** — cả 2 tuyên bố trong comment đều sai so với UI thật (xem F1). Giải quyết bằng xoá route (comment sai đã xoá theo file) thay vì thêm entry point — khuyến nghị ban đầu không còn phù hợp vì F1 tự hết hiệu lực; `ProductionJobDetailHeader.tsx:33-35` đã viết lại theo lối vào thật (BE tự sinh phiếu).                                                                                                                                                                                            |
...
| 5   | ✅ Đạt (xác nhận F1/F2/F6/F8/F9/F13) | Sau Pha 4, 2 dòng "Lắp ráp thành phẩm" của `SP0001` báo cáo đủ SL → Job `Chờ QC` → "Yêu cầu OQC" → tạo `OQC-2026-00002` → nhập AQL (Mức II, 2.5%, dùng gợi ý n=5, NG=0) → PASS → Job `Chờ giao hàng` → gõ URL tay `/manage/inventory-receipts/create-from-job?productionJobId=...` (xác nhận F1, thời điểm test route còn tồn tại) → phiếu tự tạo sẵn `PNK-2026-00004` → "Xác nhận nhập kho" → `POSTED`, tồn TP `SP0001` tăng 20 (từ 20 lên 40) → Job `Hoàn thành`. Rồi tạo `DO-260831-002` (chọn dòng `SO0002`/`JOB0002`) → Gửi duyệt → Duyệt → "Xác nhận đã giao" → `Đã giao`, tự sinh `PXK-2026-00004`, trừ tồn TP → `SO0002` → `Hoàn thành` (100%). Nhân tiện xử lý luôn `DO-260831-001` (nháp có sẵn từ trước, cho `SO0001`/`JOB0001`) qua cùng chu trình → `SO0001` cũng về `Hoàn thành`. | Đóng trọn 2 đơn hàng (`SO0001` và `SO0002` — đơn của Pha 1) về `Hoàn thành`, xác nhận toàn bộ chuỗi auto-generation từ đầu tới cuối. F1 xác nhận sống lúc test: route hoạt động đúng khi gõ URL tay nhưng đúng là mồ côi trên UI. **Từ 01/09 bước này đã tự động** (BE tự sinh phiếu `PENDING_RECEIPT` khi OQC đóng coverage) và route mồ côi đã xoá 03/09 — xem F1. |

**Không pha nào bị chặn hoàn toàn** — cả 3 chuỗi nghiệp vụ (Bán hàng, Mua vật tư, Sản xuất) đi hết
từ đầu tới cuối, kể cả nhánh gia công ngoài. Điểm nghẽn thật duy nhất lúc test (F1: phải gõ URL tay
ở Pha 5) không chặn được vì test biết trước route đích; người dùng thường sẽ kẹt cứng ở đây — đây
là phát hiện quan trọng nhất của đợt test đó. **Đã xử lý 2026-09-03** — xem ghi chú ở F1.
