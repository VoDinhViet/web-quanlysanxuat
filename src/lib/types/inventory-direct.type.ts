...
/** Mirrors the backend's InventoryDirectResDto (GET /api/inventory-directs) — field names
 *  match the backend 1:1 (not translated to friendlier FE names). No `type` (route is already
 *  DIRECT-only, backend stopped sending it). `reserved` = Σ phiếu lãnh vật tư `APPROVED` (mọi `type`);
...
export type DirectInventoryItem = {
  id: string
  code: string
  name: string
  unit: Unit
