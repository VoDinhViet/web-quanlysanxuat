...
export type BomItem = {
  ...
  type: BomItemType
  // Vật tư node DIRECT trỏ tới; null với node COMPONENT (không phải một item).
  itemId: string | null
  // COMPONENT: nhập tay trên node; DIRECT: mã/tên item liên kết.
  code: string
  name: string
  // Chỉ node DIRECT có (đọc từ item liên kết); null với node COMPONENT.
  revision: string | null
  // DIRECT: ảnh của item liên kết; COMPONENT: ảnh riêng gán trên node (`imageFileId`, upload
  ...
  // bomItemOperationsQueryOptions cho từng node) — DIRECT luôn rỗng.
  operations: ProductOperation[]
  // true = vật tư ngoài cấu trúc: `parentId` là node chủ (null = gắn Cấp 0), `path` mượn của chủ.
  isOffStructure: boolean
}

// Mirrors the backend's BomDirectResDto (GET .../bom/items/:bomItemId/directs) — vật tư
// (DIRECT) gắn trực tiếp vào một node cha, đọc riêng qua query phân trang/tìm kiếm được thay
// vì lọc client-side từ cây đầy đủ. `itemId` ở đây luôn có giá trị (khác `BomItem.itemId` có thể
// null) — mọi dòng trả về từ endpoint này chắc chắn là DIRECT trỏ item thật.
export type BomDirect = {
  id: string
  itemId: string
  code: string
  revision: string | null
