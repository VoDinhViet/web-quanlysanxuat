...
/** Mirrors the backend's InventoryConsumableResDto (GET /api/inventory-consumables) — field names
 *  match the backend 1:1 (not translated to friendlier FE names). No `type` (route is already
 *  CONSUMABLE-only, backend stopped sending it). `reserved` = Σ phiếu lãnh vật tư `APPROVED` (mọi `type`);
