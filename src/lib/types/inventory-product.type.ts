 ...
 *  (CONSUMABLE-only concepts, backend never sends them here) and no `status` — the backend deliberately
...
export type ProductInventoryItem = {
   ...
   *  `ConsumableInventoryItem.reserved` (CONSUMABLE: Σ phiếu lãnh vật tư APPROVED) — nguồn chứng từ khác nhau. */
  reserved: number
