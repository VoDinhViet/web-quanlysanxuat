 ...
 *  (DIRECT-only concepts, backend never sends them here) and no `status` — the backend deliberately
...
export type ProductInventoryItem = {
   ...
   *  `DirectInventoryItem.reserved` (DIRECT: Σ phiếu lãnh vật tư APPROVED) — nguồn chứng từ khác nhau. */
  reserved: number
