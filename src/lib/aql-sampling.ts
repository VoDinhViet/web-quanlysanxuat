...
// trước (preview) khi user đang chỉnh input ở IqcAqlInputCard/OqcAqlInputCard; kết quả xác nhận
// thật luôn do server tính lại (POST /iqc/:iqcId/confirm, POST /oqc/:oqcId/confirm — cùng bảng
// AQL dùng chung cho cả 2 domain). Sửa 2 bảng dưới đây thì nhớ sửa cả bản backend.
type LotSizeRange = { max: number } & Record<IqcInspectionLevel, string>

const lotSizeCodeLetter: LotSizeRange[] = [
...
const samplingPlan: Record<string, SamplingPlan> = {
...
export function resolveAqlPlan(
  ...
  const range = lotSizeCodeLetter.find((row) => lotSize <= row.max)
  const codeLetter = range?.[level]
  const plan = codeLetter ? samplingPlan[codeLetter] : undefined
