...
import { Box, ClipboardList, LayersMinimalistic } from "@solar-icons/react"
...
type BomItemDetailTabItem = {
  value: BomItemDetailTab
  label: string
...
// two-column grid that each panel shares with the sidebar. Same strip as
// ProductDetailTabs, including the locked-tab/tooltip treatment.
export function BomItemDetailTabs({
  lockedTabs = [],
  lockedHint,
}: BomItemDetailTabsProps) {
  return (
    <div className="border-b border-border print:hidden">
      ...
      >
        {bomItemDetailTabItems.map((item) => {
