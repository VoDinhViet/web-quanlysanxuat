...
export function ProductsTableFilter() {
  ...
  const handleExport = () => {
    toast.promise(
      ...
      {
        loading: "Đang xuất file Excel danh sách sản phẩm...",
        success: "Đã xuất file Excel danh sách sản phẩm",
  ...
  const resetFilters = () => {
    ...
    void navigate({
      search: (prev) => {
        const { q: _q, status: _status, order: _order, ...rest } = prev
        return { ...rest, page: 1 }
      },
    })
  }
  ...
  return (
    <div className="flex flex-col gap-4 bg-card px-4 py-4 lg:px-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-end lg:justify-between">
        <div className="grid flex-1 grid-cols-1 items-end gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(14rem,1.6fr)_minmax(8rem,0.8fr)]">
          <div className="space-y-1.5 sm:col-span-2 lg:col-span-1">
            ...
            <div className="relative">
              <Input
                ...
                placeholder="Tìm theo mã, phiên bản, tên sản phẩm..."
                value={q}
                onChange={(event) => {
