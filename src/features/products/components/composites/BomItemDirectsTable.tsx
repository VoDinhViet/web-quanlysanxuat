...
import { bomItemDirectsQueryOptions } from "@/features/products/api/options"
...
type BomItemDirectsTableProps = {
  productId: string
  bomItem: BomItem
  // true khi bomItem còn cấu trúc con (COMPONENT) bên dưới — vật tư thêm ở đây là vật tư ngoài
  // cấu trúc (`isOffStructure`), BE không cho gắn vật tư thường cạnh node COMPONENT (`E273`).
  hasChildPart: boolean
...
// không còn hiển thị dòng DIRECT nào nữa). Đọc qua bomItemDirectsQueryOptions
...
// node. Thêm vật tư qua CreateDirectDialog (cùng khuôn với
...
// lại nếu lỡ tay.
export function BomItemDirectsTable({
  productId,
  bomItem,
  hasChildPart,
  bom,
}: BomItemDirectsTableProps) {
  ...
  const query = useQuery({
    ...bomItemDirectsQueryOptions(productId, bomItem.id, {
      page,
      limit: PAGE_SIZE,
  ...
  function startEditing(
    ...
    setEditingDirectId(directId)
    setDraftQuantity(quantity)
    setDraftNote(note ?? "")
  }

  function saveEditing(directId: string) {
    if (draftQuantity === undefined || draftQuantity <= 0) return
    bom.updateItem({ quantity: draftQuantity, note: draftNote }, directId, () =>
        ...
        <Table aria-label="Danh sách vật tư">
          ...
          <TableBody>
            {directs.length === 0 ? (
              ...
              directs.map((direct, idx) => {
                const isEditing = editingDirectId === direct.id

                return (
                  <TableRow key={direct.id} id={direct.id} className="h-14">
                    ...
                    <TableCell className="font-medium text-muted-foreground">
                      {direct.unit ? (
                        <span title={direct.unit.code}>{direct.unit.name}</span>
                      ) : (
                    ...
                    <TableCell className="text-center">
                      {isEditing ? (
                        ...
                        <span className="font-semibold text-foreground tabular-nums">
                          {quantityFormatter.format(direct.quantity)}
                        </span>
                      )}
                    </TableCell>
                    <TableCell>
                          ...
                          {isEditing ? (
                            ...
                            <>
                              ...
                              <Button
                                ...
                                onClick={() => bom.deleteItem(direct.id)}
                              >
