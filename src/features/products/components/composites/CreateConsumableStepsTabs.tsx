...
export type CreateConsumableWizardStep = "select" | "details"

type CreateConsumableStepItem =
  WizardStepNavItem<CreateConsumableWizardStep> & {
    label: string
    icon: ComponentType<IconProps>
  }

export const createConsumableStepItems: CreateConsumableStepItem[] = [
  {
    value: "select",
    label: "1. Chọn vật tư",
    icon: Box,
...
type CreateConsumableStepsTabsProps = {
  // Bước ② chỉ mở khi đã chọn ít nhất 1 vật tư — xem CreateConsumableDialog.tsx's `picked.size`.
  canGoToDetails: boolean
  // Hiện badge số lượng đã chọn ngay trên tab ② — chỉ chỗ này cần con số này, không lặp lại ở
  // thanh tìm kiếm của bước ① (ConsumablesPickerTable.tsx) nữa.
  pickedCount: number
}

// Chỉ vẽ dải trigger — Tabs root + TabsContent panel sống ở CreateConsumableDialog.tsx, cùng cách
...
export function CreateConsumableStepsTabs({
  canGoToDetails,
  pickedCount,
}: CreateConsumableStepsTabsProps) {
  return (
    <div className="border-b border-border">
      ...
      >
        {createConsumableStepItems.map((item) => {
