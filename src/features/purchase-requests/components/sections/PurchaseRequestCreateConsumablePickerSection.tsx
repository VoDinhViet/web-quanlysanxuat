...
import { buildPurchaseRequestConsumablePickerColumns } from "@/features/purchase-requests/components/composites/PurchaseRequestCreateConsumablePickerColumns"
...
// Only ACTIVE consumables are pickable — an inactive vật tư can't be proposed for purchase. Not
// user-facing: unlike ConsumablesTableFilter's status dropdown, this picker fixes the filter
...
export const PurchaseRequestCreateConsumablePickerSection = withForm({
  ...
  render: function Render({ form, disabled }) {
    ...
    const toggleRow = useCallback(
      (consumable: Consumable) => {
        ...
        if (index >= 0) {
          ...
          itemsField.pushValue(buildPickedPurchaseRequestItem(consumable))
        }
      },
      [items, itemsField]
    )
    ...
    const columns = useMemo(
      () =>
        buildPurchaseRequestConsumablePickerColumns({
          pickedIds,
          disabled,
          allChecked,
