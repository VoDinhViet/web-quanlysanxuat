...
import { buildPurchaseRequestDirectPickerColumns } from "@/features/purchase-requests/components/composites/PurchaseRequestCreateDirectPickerColumns"
...
// Only ACTIVE directs are pickable — an inactive vật tư can't be proposed for purchase. Not
// user-facing: unlike DirectsTableFilter's status dropdown, this picker fixes the filter
...
export const PurchaseRequestCreateDirectPickerSection = withForm({
  ...
  render: function Render({ form, disabled }) {
    ...
    const toggleRow = useCallback(
      (direct: Direct) => {
        ...
        if (index >= 0) {
          ...
          itemsField.pushValue(buildPickedPurchaseRequestItem(direct))
        }
      },
      [items, itemsField]
    )
    ...
    const columns = useMemo(
      () =>
        buildPurchaseRequestDirectPickerColumns({
          pickedIds,
          disabled,
          allChecked,
