...
const dispositionOptions: RadioCardOption<OqcDisposition>[] = [
  ...
  },
  {
    ...
    icon: Sledgehammer,
    activeClassName: "border-blue-500 bg-blue-50 dark:bg-blue-500/10",
    chipClassName:
...
export function OqcDispositionCard({
  form,
  disabled,
}: OqcDispositionCardProps) {
  return (
            ...
            if (disposition === OqcDisposition.REWORK) {
              return (
                <p className="rounded-lg bg-blue-50 px-3 py-2.5 text-xs font-medium text-blue-700 dark:bg-blue-500/10 dark:text-blue-400">
                  → Chuyển "Đang rework" — trả xưởng sửa lại, QC lấy mẫu và xác
                  nhận lại trên cùng phiếu này tới khi PASS.
                </p>
              )
            }
            ...
            if (disposition === OqcDisposition.SCRAP) {
              return (
                <p className="rounded-lg bg-destructive/10 px-3 py-2.5 text-xs font-medium text-destructive">
                  ✗ Hoàn thành ngay — loại bỏ hẳn lô hàng, giải phóng lại quota
                  lô của công đoạn.
                </p>
              )
            }
