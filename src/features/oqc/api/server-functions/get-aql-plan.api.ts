...
const getOqcAqlPlanSchema = z.object({
...
export const getOqcAqlPlan = createServerFn({ method: "GET" })
  .validator(getOqcAqlPlanSchema)
  .handler(async ({ data }): Promise<AqlPlan | undefined> => {
    try {
      const response = await http.get<AqlPlan>("/api/oqc/aql-plan", {
        params: data,
      })

      return response.data
    } catch (error) {
      logHttpError(error, "getOqcAqlPlan")
