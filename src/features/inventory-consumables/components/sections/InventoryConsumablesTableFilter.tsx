...
import type { InventoryStatus } from "@/lib/types/inventory-consumable.type"
import { inventoryStatusLabels } from "@/lib/types/inventory-consumable.type"
...
export function InventoryConsumablesTableFilter() {
  // useSearch keys off the file-based route id; useNavigate's `from` keys off the
  // resolved URL path — same split InventoryConsumablesPage uses for its own copy.
  const search = useSearch({ from: "/(authed)/manage_/inventory-consumables/" })
  const navigate = useNavigate({ from: "/manage/inventory-consumables/" })
  ...
  return (
    <div className="flex flex-col gap-3 bg-card px-4 py-3.5 lg:px-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
        <div className="grid flex-1 grid-cols-1 items-end gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(12rem,1.5fr)_minmax(8rem,1fr)_minmax(8rem,1fr)_minmax(9rem,1fr)]">
          {/* Tìm kiếm */}
          <div className="space-y-1.5 sm:col-span-2 lg:col-span-1">
            <Label
              htmlFor="inventory-consumables-search"
            ...
            <div className="relative">
              <Input
                id="inventory-consumables-search"
