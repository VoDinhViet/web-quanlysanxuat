...
import { InventoryConsumableStatusBadge } from "@/features/inventory-consumables/components/primitives/InventoryConsumableStatusBadge"
import {
  ConsumableImageCell,
  ConsumableQuantityCell,
} from "@/features/inventory-consumables/components/primitives/InventoryConsumableTableCells"
...
export const inventoryConsumableColumns = inventoryColumnHelper.columns([
  ...
  inventoryColumnHelper.display({
    ...
    cell: ({ row }) => <ConsumableImageCell item={row.original} />,
  }),
  ...
  inventoryColumnHelper.accessor("onHand", {
    ...
    cell: ({ getValue }) => (
      <ConsumableQuantityCell value={getValue()} variant="blue" />
    ),
  }),
  ...
  // inventory-consumable.type.ts) — vẫn gộp thành dòng phụ nhỏ dưới số Tồn khả dụng thay vì tách
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col items-end gap-0.5">
          <ConsumableQuantityCell value={item.available} variant="available" />
  ...
  inventoryColumnHelper.display({
    ...
    cell: ({ row }) => (
      <InventoryConsumableStatusBadge
