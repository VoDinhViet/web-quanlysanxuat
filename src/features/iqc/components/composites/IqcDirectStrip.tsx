...
export function IqcDirectStrip({ iqc }: IqcDirectStripProps) {
  return iqc.item ? (
    <IqcItemDirectStrip iqc={iqc} itemRef={iqc.item} />
  ) : (
    <IqcSnapshotDirectStrip iqc={iqc} />
  )
}

type IqcItemDirectStripProps = {
  iqc: IqcDetail
...
// $iqcId.tsx).
function IqcItemDirectStrip({ iqc, itemRef }: IqcItemDirectStripProps) {
...
function IqcSnapshotDirectStrip({ iqc }: IqcDirectStripProps) {
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
