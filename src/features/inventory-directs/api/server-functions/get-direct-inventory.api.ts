...
import type { DirectInventoryItem } from "@/lib/types/inventory-direct.type"
import { optional } from "@/lib/zod-transforms"

const GENERIC_ERROR_MESSAGE = "Đã có lỗi xảy ra. Vui lòng thử lại."

function resolveGetDirectInventoryErrorMessage(error: unknown): string {
...
// `asOfDate` means "tồn tại thời điểm 23:59 ngày này" (see GetInventoryDirectsReqDto's own doc
...
const getDirectInventorySchema = z
  .object({
...
export const getDirectInventory = createServerFn({ method: "GET" })
  .validator(getDirectInventorySchema)
  .handler(
    async ({ data }): Promise<PaginatedResponse<DirectInventoryItem>> => {
      try {
        // `/api/inventory-directs` chỉ trả DIRECT (be-quanlysanxuat's InventoryDirectsService) —
        // không còn cần tự gửi `itemType` như route `/api/inventory` cũ.
        const response = await http.get<PaginatedResponse<DirectInventoryItem>>(
          "/api/inventory-directs",
          { params: data }
        )

        return response.data
      } catch (error) {
        logHttpError(error, "getDirectInventory")

        throw new Error(resolveGetDirectInventoryErrorMessage(error))
      }
    }
  )
