...
import { InventoryDirectStatusBadge } from "@/features/inventory-directs/components/primitives/InventoryDirectStatusBadge"
import {
  DirectImageCell,
  DirectQuantityCell,
} from "@/features/inventory-directs/components/primitives/InventoryDirectTableCells"
import type { DirectInventoryItem } from "@/lib/types/inventory-direct.type"
import { resolveInventoryStatus } from "@/lib/types/inventory-direct.type"
...
export const inventoryDirectColumns = inventoryColumnHelper.columns([
  ...
  inventoryColumnHelper.display({
    ...
    cell: ({ row }) => <DirectImageCell item={row.original} />,
  }),
  ...
  inventoryColumnHelper.accessor("onHand", {
    ...
    cell: ({ getValue }) => (
      <DirectQuantityCell value={getValue()} variant="blue" />
    ),
  }),
  ...
  // inventory-direct.type.ts) — vẫn gộp thành dòng phụ nhỏ dưới số Tồn khả dụng thay vì tách
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col items-end gap-0.5">
          <DirectQuantityCell value={item.available} variant="available" />
