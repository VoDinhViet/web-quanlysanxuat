...
function resolveUploadMaterialImageErrorMessage(error: unknown): string {
...
export const uploadMaterialImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ url: string }> => {
    try {
      ...
      logHttpError(error, "uploadMaterialImage")

      throw new Error(resolveUploadMaterialImageErrorMessage(error))
    }
  })
