...
type CreateOrderSelectItemsStepProps = {
  form: UseFormReturn<CreateOrderSchema>
}

// Bước ② của wizard: danh mục sản phẩm có ảnh/tồn kho, tìm kiếm + phân trang, tick chọn nhiều —
// tick là ghi thẳng vào `items` field-array của form (append/remove qua useFieldArray), không còn
// state "đang chọn tạm" riêng như bản popup cũ (OrderItemsPickerDialog.tsx, xem lịch sử). Bỏ tick
// 1 dòng đang có trong đơn tương đương xoá dòng đó khỏi đơn.
export function CreateOrderSelectItemsStep({
  form,
}: CreateOrderSelectItemsStepProps) {
